...
- `chimefrbcat1_data.csv` — local working CSV corresponding to CHIME/FRB
  Catalog 1 data used by the current reproducible pipeline stages.
- `chimefrbcat1.csv` — expected raw download filename for CHIME/FRB Catalog 1
  if generated by Stage 1; validate before use.
...
- `catalog1.csv` and `repeaters.csv` — legacy/raw catalog filenames that may be
  produced during manual data acquisition or earlier exploratory work; validate
  against the CHIME schema before scientific use.
